...
public class DocumentRepository(AppDbContext appDbContext) : IDocument
{
    ...
    public async Task<List<Documento>> GetAllDocumentosDateRange(string start, string end)
    {
        var documentos = await appDbContext.Documentos.ToListAsync();
    ...
    public async Task<GeneralResponse> CreateDocumento(AsientoDto item)
{
    ...
    if (item.Cabecera == null) return new GeneralResponse(false, "La cabecera del asiento contable no puede ser vacia");
    if (string.IsNullOrEmpty(item.Cabecera.NroDoc)) return new GeneralResponse(false, "El número de documento es requerido");
    ...
    foreach (var detalle in item.Detalle)
    {
        ...
        if (detalleDocumento == null) return new GeneralResponse(false, "Error al procesar el Detalle del documento y asociarlo a la cabecera");
    ...
    private Documento CheckChanges(Documento newDoc, Documento oldDoc)
    {
        ...
        if (newDoc.ProveedorCodProveedor != oldDoc.ProveedorCodProveedor) oldDoc.ProveedorCodProveedor = newDoc.ProveedorCodProveedor;
